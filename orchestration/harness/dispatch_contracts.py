...
def validate_request(payload: Dict, failures: List[str]) -> None:
    ...
    for field in ["work_ref", "plan_ref", "revision_of_dispatch_ref", "goal_ref", "goal_step_ref"]:
        if field in payload:
            require_string(payload, field, failures, prefix="request.json ")
    if "goal_step_index" in payload:
        goal_step_index = payload.get("goal_step_index")
        if not isinstance(goal_step_index, int) or goal_step_index < 1:
            failures.append("request.json goal_step_index must be a positive integer")
