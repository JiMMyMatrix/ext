...
check "Advisory MCP Python requirements exist" "test -f orchestration/runtime/advisory/requirements.txt && rg -n '^anthropic|^mcp' orchestration/runtime/advisory/requirements.txt > /dev/null"
check "Advisory MCP root entrypoint exists" "test -f mcp_server.py"
check "Development consulting MCP entrypoint exists" "test -f dev_mcp_server.py && test -f orchestration/scripts/serve_development_consulting_mcp.py"
check "Advisory MCP launcher exists" "test -f orchestration/scripts/serve_advisory_mcp.py && test -f orchestration/scripts/setup_advisory_mcp_env.py"
check "Runtime config registers advisory MCP entrypoint" "rg -n '\\[mcp_servers\\.orchestration_advisory\\]' orchestration/runtime/config.toml > /dev/null && rg -n 'args = \\[\"mcp_server\\.py\"\\]' orchestration/runtime/config.toml > /dev/null"
check "Advisory MCP launcher handles Python environment" "rg -n 'ORCHESTRATION_APPROVED_PYTHON|CORGI_PYTHON|/opt/homebrew/bin/python3|PYTHONPATH|mcp_server\\.py|requirements\\.txt|CORGI_ADVISORY_CONTEXT|ORCHESTRATION_SOURCE_ROOT' orchestration/scripts/serve_advisory_mcp.py > /dev/null"
...
check "Advisor MCP skills exist" "test -f orchestration/skills/routing/SKILL.md && test -f orchestration/skills/claude-headless/SKILL.md && test -f orchestration/skills/consult-architect/SKILL.md && test -f orchestration/skills/routine-code-review/SKILL.md && test -f orchestration/skills/minimax-advisor/SKILL.md"
check "Advisor MCP skills reference server tools" "rg -n 'consult_claude_headless' orchestration/skills/claude-headless/SKILL.md orchestration/skills/routing/SKILL.md > /dev/null && rg -n 'consult_architect' orchestration/skills/consult-architect/SKILL.md orchestration/skills/routing/SKILL.md > /dev/null && rg -n 'routine_code_review' orchestration/skills/routine-code-review/SKILL.md orchestration/skills/routing/SKILL.md > /dev/null && rg -n 'consult_minimax' orchestration/skills/minimax-advisor/SKILL.md orchestration/skills/routing/SKILL.md > /dev/null"
check "Advisor usage is cost-gated" "rg -n 'cost-gated|prefer one advisor|wrong decision would cost more than two consultations|default to no advisor' orchestration/prompts/governor.txt orchestration/advisory.md orchestration/workflow.md orchestration/skills/routing/SKILL.md orchestration/skills/governor-workflow/SKILL.md > /dev/null"
