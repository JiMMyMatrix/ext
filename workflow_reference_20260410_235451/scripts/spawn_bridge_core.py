...
from scripts.dispatch_start_guard import ensure_dispatch_startable
from scripts.overlap_worktree import prepare_overlap_worktree, requested_overlap_isolation
...
def _load_request(repo_root: Path, dispatch_ref: str) -> tuple[Path, Dict[str, Any]]:
    ...
    if not request_path.exists():
        raise SystemExit(f"dispatch request does not exist: {relative_path(request_path, repo_root)}")
...
def _load_existing_bridge_or_base(repo_root: Path, request: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if bridge_path.exists():
        ...
        if existing.get("dispatch_ref") != request["dispatch_ref"]:
            raise SystemExit("existing spawn_bridge.json dispatch_ref does not match request dispatch_ref")
        return existing
...
def _persist_bridge(repo_root: Path, bridge: Dict[str, Any]) -> Dict[str, Any]:
    ...
    payload = dict(bridge)
    payload["bridge_artifact_ref"] = relative_path(bridge_path, repo_root)
    return payload
...
def build_executor_handoff(
    ...
    *,
    ...
    dispatch_ref = request["dispatch_ref"]
    dispatch_dir = dispatch_dir_for_ref(repo_root, dispatch_ref)
    request_ref = relative_path(dispatch_dir / "request.json", repo_root)
    lines = [
    ...
    if overlap_payload is not None:
        lines.extend(
            [
                ...
                f"  python3 {repo_root / 'scripts/overlap_worktree.py'} finalize-candidate --dispatch-ref {dispatch_ref} --root {repo_root}",
            ]
        )
    lines.append("Next action: spawn the executor in the live chat window using .codex/agents/executor.toml.")
    return "\n".join(lines) + "\n"
...
def build_reviewer_handoff(request: Dict[str, Any], result: Dict[str, Any], repo_root: Path) -> str:
    ...
    dispatch_dir = dispatch_dir_for_ref(repo_root, dispatch_ref)
    request_ref = relative_path(dispatch_dir / "request.json", repo_root)
    result_ref = relative_path(dispatch_dir / "result.json", repo_root)
    lines = [
    ...
    lines.append("Next action: spawn the reviewer in the live chat window using .codex/agents/reviewer.toml.")
    return "\n".join(lines) + "\n"
...
def prepare_executor_spawn(repo_root: Path, dispatch_ref: str) -> Dict[str, Any]:
    ...
    bridge["last_action"] = "prepare_executor_spawn"
    bridge["executor_handoff_ref"] = relative_path(handoff_path, repo_root)
    bridge["overlap_isolation_ref"] = overlap_payload.get("artifact_ref") if overlap_payload else None
    persisted = _persist_bridge(repo_root, bridge)
    persisted["spawn_required"] = True
    persisted["next_action"] = "spawn executor in the live chat window using .codex/agents/executor.toml"
    return persisted
...
def record_executor_spawn(
    ...
    *,
    ...
    persisted = _persist_bridge(repo_root, bridge)
    persisted["next_action"] = "wait_for_executor_result" if outcome == "spawned" else "executor spawn still required"
    return persisted
...
def prepare_reviewer_spawn(repo_root: Path, dispatch_ref: str) -> Dict[str, Any]:
    ...
    bridge["last_action"] = "prepare_reviewer_spawn"
    bridge["reviewer_handoff_ref"] = relative_path(handoff_path, repo_root)
    persisted = _persist_bridge(repo_root, bridge)
    persisted["spawn_required"] = True
    persisted["next_action"] = "spawn reviewer in the live chat window using .codex/agents/reviewer.toml"
    return persisted
...
def record_reviewer_spawn(
    ...
    *,
    ...
    persisted = _persist_bridge(repo_root, bridge)
    persisted["next_action"] = "persist review.json, then finalize" if outcome == "spawned" else "reviewer spawn still required"
    return persisted
