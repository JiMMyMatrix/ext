...
export function getExecutionWindowClientScript(
	resetPersistedState: boolean,
	testWindowAutoStepMode: TestWindowAutoStepMode
): string {
	return `		const vscode = acquireVsCodeApi();
		const shouldResetPersistedState = ${resetPersistedState ? 'true' : 'false'};
		const testWindowAutoStepMode = ${JSON.stringify(testWindowAutoStepMode)};
