...
from scripts.governor_transition import ALLOWED_CONTINUE_ACTIONS, ALLOWED_HUMAN_STOP_REASONS, TRANSITION_TYPES
from scripts.reviewer_contract import ReviewerContractViolation, resolve_review_artifact_path
from scripts.overlap_worktree import INTEGRATION_POLICIES, OVERLAP_ISOLATION_MODE
from scripts.spawn_bridge_core import BRIDGE_STAGES, HELPER_RUNTIME_PATH, LIVE_SUBAGENT_MODES, LIVE_SUBAGENT_PATH
...
def validate_executor_run(payload: Dict, failures: List[str]) -> None:
    if not isinstance(payload, dict):
        ...
        return
    for field in ["run_ref", "objective", "scope", "read_list", "produce_list", "planned_file_touch_list", "non_goals", "stop_conditions"]:
        if field not in payload:
            failures.append(f"executor_run missing field: {field}")
    for field in ["read_list", "produce_list", "planned_file_touch_list", "non_goals", "stop_conditions"]:
...
def validate_human_escalation_policy(payload: Dict, failures: List[str]) -> None:
    ...
    if "allowed_without_advisors" in payload and not isinstance(payload["allowed_without_advisors"], list):
...
def validate_batch_context(payload: Dict, failures: List[str]) -> None:
    ...
    if "prior_batch_checkpoints" in payload and not isinstance(payload["prior_batch_checkpoints"], list):
...
def validate_execution_plan(payload: Dict, failures: List[str]) -> None:
    ...
    if "total_estimated_minutes" not in payload or not isinstance(payload.get("total_estimated_minutes"), int):
        failures.append("execution_plan.total_estimated_minutes must be an integer")
    if "checkpoint_after_step" not in payload or not isinstance(payload.get("checkpoint_after_step"), int):
...
def validate_review_fields(payload: Dict, failures: List[str]) -> None:
    ...
    if isinstance(review_artifact_path, str) and review_artifact_path.strip():
        try:
            resolve_review_artifact_path(Path("."), payload.get("dispatch_ref", "cycle/unknown/task/a01"), review_artifact_path)
...
def validate_overlap_isolation_request(payload: Dict, failures: List[str]) -> None:
    ...
    if integration_policy not in INTEGRATION_POLICIES:
        failures.append("request.json overlap_isolation.integration_policy must be choose_one or can_stack")
    if payload.get("execution_mode") not in LIVE_SUBAGENT_MODES:
        failures.append("request.json overlap_isolation is supported only for live-subagent execution modes")
...
def validate_execution_payload(request: Dict, failures: List[str]) -> None:
    ...
    if execution_mode == "sample_acceptance":
        ...
        if isinstance(sample_id, str) and sample_id not in SUPPORTED_SAMPLE_ACCEPTANCE_IDS:
            ...
            failures.append(
                "sample_acceptance execution_payload.sample_id must be one of: "
                f"{supported}"
            )
    if execution_mode == "guided_agent":
        ...
        require_string_list(payload, "injected_weakness_guards", failures, prefix="guided_agent execution_payload.")
    if execution_mode == "strict_refactor":
        ...
        require_string_list(payload, "injected_weakness_guards", failures, prefix="strict_refactor execution_payload.")
...
def validate_request(payload: Dict, failures: List[str]) -> None:
    ...
    checkpoint_outputs = [
        item for item in payload.get("required_outputs", []) if isinstance(item, str) and "checkpoint" in Path(item).name
    ]
    ...
    if estimated_complexity in {"medium", "high"} and "execution_plan" not in payload:
        failures.append("request.json execution_plan is required when estimated_complexity is medium/high")
    if (estimated_complexity in {"medium", "high"} or isinstance(batch_context, dict)) and not checkpoint_outputs:
        failures.append("request.json must declare a checkpoint artifact for medium/high or batched dispatches")
...
def validate_result(payload: Dict, failures: List[str]) -> None:
    ...
    for field in ["executor_run_refs", "written_or_updated", "auto_validated", "notes"]:
        require_list(payload, field, failures)
    if "runtime_behavior_changed" in payload and not isinstance(payload["runtime_behavior_changed"], bool):
...
def validate_governor_decision(payload: Dict, failures: List[str]) -> None:
    ...
    if decision is not None and decision not in DECISION_VALUES:
        failures.append("governor_decision.json decision must be one of: accept, reject, needs_review, needs_verification")
...
def validate_spawn_bridge(
    ...
    *,
    ...
    if bridge_stage not in BRIDGE_STAGES:
        failures.append(f"spawn_bridge.json bridge_stage must be one of: {', '.join(sorted(BRIDGE_STAGES))}")
    ...
    for field in ["executor_handoff_ref", "reviewer_handoff_ref"]:
        ...
        if value is not None and (not isinstance(value, str) or not value.strip()):
            failures.append(f"spawn_bridge.json {field} must be a non-empty string when present")
    ...
    if overlap_isolation_ref is not None and (
        ...
        failures.append("spawn_bridge.json overlap_isolation_ref must be a non-empty string when present")
    ...
    if isinstance(execution_mode, str) and isinstance(resolved_path, str):
        expected_path = LIVE_SUBAGENT_PATH if execution_mode in LIVE_SUBAGENT_MODES else HELPER_RUNTIME_PATH
...
def validate_overlap_isolation_artifact(
    ...
    *,
    ...
    if payload.get("integration_policy") not in INTEGRATION_POLICIES:
        failures.append("overlap_isolation.json integration_policy must be choose_one or can_stack")
    if request is not None:
        ...
        if payload.get("originating_dispatch_ref") != request.get("dispatch_ref"):
            failures.append("originating_dispatch_ref mismatch between request.json and overlap_isolation.json")
        ...
        if not isinstance(request_overlap, dict):
            failures.append("overlap_isolation.json exists but request.json does not declare overlap_isolation")
        else:
            ...
            if payload.get("integration_policy") != request_overlap.get("integration_policy"):
                failures.append("integration_policy mismatch between request.json and overlap_isolation.json")
...
def validate_proposed_transition(payload: Dict, failures: List[str]) -> None:
    ...
    if transition not in TRANSITION_TYPES:
        failures.append(
            "proposed_transition.json transition must be one of: " + ", ".join(sorted(TRANSITION_TYPES))
        )
    ...
    for field in ["dispatch_ref", "decision_ref"]:
        value = payload.get(field)
        if value is not None and (not isinstance(value, str) or not value.strip()):
            failures.append(f"proposed_transition.json {field} must be a non-empty string when present")

    next_action = payload.get("next_action")
    blocker = payload.get("blocker")
    if transition == "continue_internal":
        if requested_stop_reason is not None:
            failures.append("proposed_transition.json requested_stop_reason must be null for continue_internal")
        if not isinstance(next_action, dict):
            failures.append("proposed_transition.json next_action must be an object for continue_internal")
        else:
            ...
            if ref is not None and (not isinstance(ref, str) or not ref.strip()):
                failures.append("proposed_transition.json next_action.ref must be a non-empty string when present")
            summary = next_action.get("summary")
            if summary is not None and not isinstance(summary, str):
                failures.append("proposed_transition.json next_action.summary must be a string when present")
        ...
        if requested_stop_reason in {
            ...
            if not isinstance(blocker, dict):
                failures.append("proposed_transition.json blocker must be an object for blocker-style interrupts")
            else:
                ...
                if not isinstance(category, str) or not category.strip():
                    failures.append("proposed_transition.json blocker.category must be a non-empty string")
                if not isinstance(summary, str) or not summary.strip():
                    failures.append("proposed_transition.json blocker.summary must be a non-empty string")
                if not isinstance(artifact_refs, list) or not artifact_refs or any(
                    ...
                    failures.append("proposed_transition.json blocker.artifact_refs must be a non-empty list of strings")
    ...
    if not isinstance(completion_rule, dict):
        ...
        if not isinstance(mode, str) or not mode.strip():
            failures.append("proposed_transition.json completion_rule.completion_mode must be a non-empty string")
        if not isinstance(rules_ref, str) or not rules_ref.strip():
            failures.append("proposed_transition.json completion_rule.rules_ref must be a non-empty string")
...
def validate_state(payload: Dict, failures: List[str]) -> None:
    ...
    if status is not None and status not in STATE_STATUS:
        failures.append("state.json status must be one of: queued, claimed, running, validated, completed, escalated")
        ...
        for idx, item in enumerate(history):
            ...
            for field in ["from", "to", "at", "actor"]:
                if field not in item:
                    failures.append(f"state.json transition_history[{idx}] missing field: {field}")
            ...
            if to_state is not None and to_state not in allowed:
                failures.append(f"state.json transition_history[{idx}] has illegal transition {from_state!r} -> {to_state!r}")
            if previous is not None and previous != from_state:
                failures.append(f"state.json transition_history[{idx}] does not chain from previous state")
            previous = to_state
    ...
    if target.is_dir():
        ...
        if decision_path.exists():
            ...
            if dispatch_ref != decision.get("dispatch_ref"):
                failures.append("dispatch_ref mismatch between request.json and governor_decision.json")
