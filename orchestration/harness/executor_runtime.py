...
from orchestration.harness.runtime_support import (
...
def ensure_helper_runtime_dispatch(dispatch_dir: Path, request: Dict) -> None:
    ...
    if execution_mode in SUBAGENT_ONLY_MODES:
        raise SystemExit(
            ...
            f"{script_ref('executor_consume_dispatch.py', REPO_ROOT)} must not claim it"
        )
...
def scaffold_or_reuse_run(repo_root: Path, executor_run: Dict) -> Path:
    ...
    task = {
        "run_ref": run_ref,
        "objective": executor_run["objective"],
        "scope": executor_run["scope"],
        "non_goals": executor_run.get("non_goals", []),
        "stop_conditions": executor_run.get("stop_conditions", []),
    }
    status = {
        "run_ref": run_ref,
        "current_branch": current_branch,
        "git_status_short": git_status,
        "planned_file_touch_list": executor_run.get("planned_file_touch_list", []),
        "read_list": executor_run.get("read_list", []),
        "produce_list": executor_run.get("produce_list", []),
        "scope": executor_run["scope"],
        "non_goals": executor_run.get("non_goals", []),
        "stop_conditions": executor_run.get("stop_conditions", []),
        "state": "running",
...
def execute_sample_acceptance(repo_root: Path, dispatch_dir: Path, request: Dict, run_dir: Path) -> List[str]:
    del repo_root, dispatch_dir, request, run_dir
    raise SystemExit(
        "unsupported_in_orchestration_port: sample_acceptance helper runtime is not shipped in the stabilized orchestration surface"
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    ensure_running_with_approved_python(script_ref("executor_consume_dispatch.py", REPO_ROOT))
    ...
    try:
        ...
        try:
            ...
            if execution_mode == "command_chain":
                ...
                escalation_path = write_escalation(
                    ...
                    (
                        ...
                        f"{script_ref('executor_consume_dispatch.py', repo_root)}; route this dispatch through the executor "
                        "subagent path instead of the helper runtime"
                    ),
                    [
                        str((dispatch_dir / "request.json").relative_to(repo_root)),
                        prompt_ref("executor.txt", repo_root),
                        contract_ref("dispatch.md", repo_root),
                    ],
                    failure_category="unsupported_in_orchestration_port",
                )
                update_state(dispatch_dir, "escalated", "agentB", "execution mode requires subagent path")
                print(str(escalation_path.relative_to(repo_root)))
                return 1
            elif execution_mode in UNSUPPORTED_IN_ORCHESTRATION_PORT_MODES:
                escalation_path = write_escalation(
                    dispatch_dir,
                    request["dispatch_ref"],
                    "agentB",
                    (
                        f"execution_mode {execution_mode!r} is intentionally unavailable in the stabilized "
                        "orchestration helper runtime"
                    ),
                    [
                        str((dispatch_dir / "request.json").relative_to(repo_root)),
                        contract_ref("dispatch.md", repo_root),
                ...
                update_state(dispatch_dir, "escalated", "agentB", "execution mode unavailable in orchestration port")
                print(str(escalation_path.relative_to(repo_root)))
                return 1
            else:
                escalation_path = write_escalation(
            ...
            auto_validated = []
            run_command(
                [approved_python, script_ref("validate_run_contract.py", repo_root), str(run_dir)],
                repo_root,
            )
            auto_validated.append(
                f"python3 {script_ref('validate_run_contract.py', repo_root)} {run_dir.relative_to(repo_root)}"
            )
