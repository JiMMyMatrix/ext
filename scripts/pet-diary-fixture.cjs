...
function seedFilterPetDiaryApp(scratchRoot) {
	writeFixtureFile(
		scratchRoot,
		'README.md',
		`# Pet Life Diary

This seeded scratch app is ready for a small feature addition: filter visible diary entries by pet species.
...
function main(argv) {
	...
	if (!['seed-bugfix', 'seed-filter'].includes(command) || !scratchRoot) {
		process.stderr.write('Usage: node scripts/pet-diary-fixture.cjs <seed-bugfix|seed-filter> <scratch-root>\n');
		return 2;
	}
	if (command === 'seed-bugfix') {
		seedBuggyPetDiaryApp(path.resolve(scratchRoot));
	} else {
		seedFilterPetDiaryApp(path.resolve(scratchRoot));
	}
	return 0;
}
